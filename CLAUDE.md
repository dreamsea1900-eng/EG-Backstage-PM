...
3. **對焦環節**（新頁）：用幾句話**回述我讀到的理解**（逐一列出各檔角色：流程／規格／視覺參考…），
   ...
   每題附最佳猜測預設、用 AskUserQuestion 點選作答（PM 不必打字）。
4. **製作前確認**：對焦完成後、**正式切版前**，先提示預估 token %（以 1M context 為基準），
   AskUserQuestion（執行／取消），PM 選「執行」才開始 build。
...
1. **準備材料**：把流程圖、規格、參考圖丟進 `input/`（多份檔案、圖或 md 皆可；見「如何準備規格」），
   或整理成一份文字規格
2. **產出 prototype**：把資料夾拖進 Claude Code、送出任一句話 → 依「開工自動流程」自動讀 `input/`、
   分流、對焦，再走**開工模式** build（解析 → 建資料夾 → 切版互動 → 驗證 → 交付摘要）；
...
- **流程圖可以直接放圖**：把截圖丟進 `input/` 請 Claude 讀圖，或用文字把流程描述進規格
...
| `.claude/commands/` | **開工模式**（`spec-to-proto`）、**新增/修改模式**（`revise`）的流程指令（幕後引擎） |
