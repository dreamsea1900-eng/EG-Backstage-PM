...
- **指令路由**（不支援 `.claude/commands` 斜線指令的工具適用）：PM 說「**開工**」或貼規格要做新頁
  ...
  → 讀取並遵循 `.claude/commands/revise.md`。
...
3. **對焦環節**（新頁）：用幾句話**回述我讀到的理解**（逐一列出各檔角色：流程／規格／視覺參考…），
   ...
   每題附最佳猜測預設、以「選項確認」作答（PM 不必打字）。
4. **製作前確認**：對焦完成後、**正式切版前**，先提示預估 token %（基準見〈工具對應〉），
   「選項確認」（執行／取消），PM 選「執行」才開始 build。
...
| `.claude/commands/` | **開工模式**（`spec-to-proto`）、**新增/修改模式**（`revise`）的流程指令（幕後引擎）；Claude Code 走斜線指令，其他工具依〈工具對應〉的「指令路由」讀取同一份檔案 |
